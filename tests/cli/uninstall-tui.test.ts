...
import { npmInstallation, uninstallCompletely, uninstallWithUnreadableConfig } from '../../src/cli/uninstall.js';
...
vi.mock('../../src/cli/uninstall.js', () => ({ npmInstallation: vi.fn(), uninstallCompletely: vi.fn(), uninstallWithUnreadableConfig: vi.fn() }));
let home: string;
...
beforeEach(() => {
  ...
  vi.mocked(uninstallWithUnreadableConfig).mockResolvedValue();
});
afterEach(() => {
...
it('uses the data-preserving recovery path when config is malformed', async () => {
  writeFileSync(join(home, 'config.json'), '{invalid');
  vi.mocked(clack.select).mockResolvedValueOnce('complete'); vi.mocked(clack.confirm).mockResolvedValueOnce(true);
  await runUninstallTui();
  expect(uninstallWithUnreadableConfig).toHaveBeenCalledWith(expect.objectContaining({ installation: expect.any(Object) }));
  expect(uninstallCompletely).not.toHaveBeenCalled();

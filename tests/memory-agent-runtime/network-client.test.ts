...
import { NetworkClient, networkFailure } from '../../src/memory-agent-runtime/network/client.js';
import { resolveRoute } from '../../src/memory-agent-runtime/network/route.js';
import { ProviderMemoryAgent } from '../../src/memory-agent-runtime/provider.js';
...
it.each(['http','https'] as const)('distinguishes %s proxy 407 from provider 401 without leaking credentials', async protocol => {
  ...
  const error = await probeMemoryAgent(new ProviderMemoryAgent({baseUrl:endpoint,model:'fake',apiKey:'synthetic',fetch:client(endpoint,`http://user:wrong-password@127.0.0.1:${port}`).fetch,maxRetries:0}),new AbortController().signal).catch((e:unknown)=>e);
  ...
  expect((error as {diagnostic:unknown}).diagnostic).not.toHaveProperty('httpStatus'); expect(JSON.stringify(error)).not.toMatch(/wrong-password|user|secret proxy/);
  ...
  await expect(probeMemoryAgent(new ProviderMemoryAgent({baseUrl:url,model:'fake',apiKey:'synthetic',fetch:client(url).fetch,maxRetries:0}),new AbortController().signal)).rejects.toMatchObject({code:'AUTHENTICATION',diagnostic:{stage:'http',reason:'authentication',httpStatus:401}});
});
...
it('keeps the route and dispatcher across Pi request retries when environment changes', async () => {
  ...
  const first=await listen(httpServer((_q,r)=>{attempts++;env.HTTP_PROXY=`http://127.0.0.1:${second}`;r.writeHead(attempts===1?503:401,{'retry-after':'0'});r.end('{}');}));
...
it('close aborts active connections, is idempotent and blocks new requests', async () => {
  const reached=Promise.withResolvers<void>();
  const port=await listen(httpServer(()=>reached.resolve())),endpoint=`http://127.0.0.1:${port}`,network=client(endpoint);
  const active=network.fetch(endpoint).catch(e=>e);await reached.promise;
  const close=network.close();expect(network.close()).toBe(close);await close;
  expect(await active).toBeInstanceOf(Error);await expect(network.fetch(endpoint)).rejects.toMatchObject({code:'CANCELLED'});
});

...
export class Writer extends CoreWriter {
  constructor({ model, ...options }: Omit<WriterOptions, 'agent'> & { model: MemoryModelPort }) {
    super({ ...options, agent: { async decide(task, reads, run) {
      const result = await model.analyze(readTask(task, reads), { requestId: task.request_id, signal:run.signal, deadlineMs:Number.POSITIVE_INFINITY, ...(run.onDiagnosticContext ? {onDiagnosticContext:run.onDiagnosticContext} : {}) });

...
**v0.3.9** · npm 包名 `common-memory-core` · 命令 `common-memory` · MIT 许可证。
...
- 不想全局安装？可用 `npx --yes common-memory-core@0.3.9` 运行已发布版本。
...
默认配置位于 `~/.common-memory/config.json`，API Key 只能在 TUI 中配置并保存到同目录的私有 `.env`；
所有运行入口（包括旧配置）只读取该文件，不使用终端或宿主的 API Key 环境变量。修改后需重启助手。
...
Linux/macOS CI 在 Node 22 / 24 执行完整测试、构建和 npm 安装验证；Windows CI 只验证
PowerShell 薄桥接。发布前另在真实 WSL 验证安装、TUI 与宿主接入，发布后从 npm registry
安装核验。测试证明协议和本地

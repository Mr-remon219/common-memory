...
- Preserve the current authority boundary: canonical Markdown under `<dataRoot>/memory/` is the fact source, runtime SQLite is the durable queue/lease/source-link store (not a rebuildable index), remote models return `memory_maintenance_v2` decisions, and Core validates and commits writes. Pi integration is an Extension that captures user turns and injects authorized memory into the system prompt on `before_agent_start`. MCP is stdio only, with launch-fixed capability profiles (`relay`, `init`, `read`); `read` processes never open the runtime database. Other agents' summaries enter only as `agent_import` observations via `memory_init`, and user-chosen local Markdown files only as `document_import` observations via `common-memory import` (input preprocessing: validation and structural chunking, no second semantic model); both are attributed, never treated as user statements, never sole evidence for forget, batched apart from user turns, and authorized per provenance class in `disclosure.allowedProvenance`. There is no retrieval, index, HTTP transport or Recall write authority; do not add them because older notes mention them.

...
class ConfiguredWriter extends Writer {
  ...
  constructor(options: WriterOptions, model: OpenAIResponsesMemoryModel | OpenAIChatMemoryModel) { super(options); this.#model = model; }
  ...
  override close(): Promise<void> {
    ...
    return this.#closing = Promise.allSettled([...this.#runs]).then(async () => {
      try { super.close(); } finally { await this.#model.close(); }
    });
  }
}

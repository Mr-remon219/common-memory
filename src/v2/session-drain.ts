...
export async function drainSessions(writer:DrainWriter, options:{sessionId?:string;signal?:AbortSignal}={}):Promise<boolean> {
  ...
  for(;;) {
    ...
    // run coordinates saved configuration and paused-job recovery before readiness.
    ...
    if(!pending) return options.sessionId ? ingress.status(options.sessionId).complete : !writer.store.hasIncompleteWork() && !writer.store.hasBufferedSessionWork() && writer.store.db.prepare('SELECT id FROM sessions WHERE closing=1').all().every(row=>ingress.status(String(row.id)).complete);
    if(result && typeof result==='object' && 'outcome' in result) {
      if(result.outcome==='paused'||result.outcome==='cancelled')return false;
      if(result.outcome==='idle'||result.outcome==='failed')await setTimeout(250,undefined,{...(options.signal?{signal:options.signal}:{})});
    }
  }
}

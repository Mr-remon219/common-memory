...
/** A local file the user chose to import; one complete source observation/bundle (legacy multipart jobs retain their boundaries). */
...
/** @deprecated Historical Init limit; current admission uses explicit disclosure limits only. */
export const MAX_IMPORT_UNDERSTANDING_BYTES = 32768;
/** @deprecated Historical Init gap limit; not a current input cap. */
export const MAX_IMPORT_GAPS_BYTES = 4096;
export const IMPORT_LABEL_PATTERN = /^[A-Za-z0-9][A-Za-z0-9 ._:-]{0,63}$/;

export function encodeAgentImport(payload: AgentImportPayload): string {
  if (!IMPORT_LABEL_PATTERN.test(payload.sourceLabel)) throw new Error('INVALID_IMPORT_LABEL');
  if (!IMPORT_BASES.includes(payload.basis)) throw new Error('INVALID_IMPORT_BASIS');
  if (!payload.understanding.trim()) throw new Error('INVALID_TEXT_SIZE');

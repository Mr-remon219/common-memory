...
import { editResultField, type RuntimeStore } from './runtime.js';
...
export function scopedQueueStatus(store: RuntimeStore, scopes: readonly string[]) {
  ...
  const jobStates=store.db.prepare(`SELECT j.state,COUNT(*) AS count FROM jobs j WHERE ${eligible} GROUP BY j.state`).all(...scopes)
    .map(r=>({state:String(r.state),count:Number(r.count)}));
  const jobs = store.db.prepare(`SELECT j.* FROM jobs j WHERE ${eligible} ORDER BY CASE WHEN j.state='done' THEN 1 ELSE 0 END,j.rowid DESC LIMIT 20`).all(...scopes).map(r => {
    let diagnostic = null;
    try { diagnostic = sanitizeDiagnostic(JSON.parse(String(r.diagnostic))); } catch { /* No untrusted diagnostic text. */ }
    return {...editResultField(r.editResult),id:String(r.id),state:String(r.state),attempts:Number(r.attempts),diagnostic,retryAt:r.state==='retry'?Number(r.available):null};
  });
  return {observations,jobStates,jobs};
}
export type QueueJob = {editResult?: import('./contract.js').EditResult;id:string;state:string;attempts:number;diagnostic:ReturnType<typeof sanitizeDiagnostic>;retryAt:number|null};

...
export const statusOutput = z.object({
  ...
  limits:z.object({maxInputBytes:z.number().nullable(),maxMessageBytes:z.number()}).strict().optional(),
...
export function toolFailure(error: unknown) {
  const code = error instanceof Error && Object.hasOwn(errorHelp,error.message) ? error.message : 'MEMORY_UNAVAILABLE';

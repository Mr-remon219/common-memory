...
export function createCommonMemoryPiExtension(options: {runtimeFactory?: () => PiCaptureRuntime; serviceFactory?:()=>PiMemoryService; resolveScope?: (cwd:string) => string; configFactory?: () => CommonMemoryConfig | null} = {}) {
  return (pi: ExtensionAPI): void => {
    ...
    const unregisterInstance = registerRuntimeInstance({ role: 'pi', version: currentRuntimeVersion(), pid: process.pid, executable: process.execPath, cli: new URL('./index.js', import.meta.url).pathname, lifecycle:'channel', wireProtocol:1 });
    ...
    const get = (): PiCaptureRuntime => {
      ...
      return runtime = new PiCaptureRuntime();
    };
    const safe = (fn:(r:PiCaptureRuntime)=>void|Promise<void>): void => { try { void Promise.resolve(fn(get())).catch(error=>report("capture",error)); } catch (error) { report("capture",error); } };
    const bind = (ctx:ExtensionContext): void => safe(r=>r.bind(ctx.sessionManager.getSessionId(),branchUsers(ctx.sessionManager.getBranch()),ctx.cwd));
    ...
    const service=options.serviceFactory?.()??new PiMemoryService({config:freshConfig});
    ...
    const updateFeedback=async()=>{
      const ctx=uiContext,generation=feedbackGeneration;if(!ctx?.hasUI||feedbackRunning)return;
      feedbackRunning=true;
      try {
        const status=await service.status(host(ctx));if(generation!==feedbackGeneration||!('queue' in status))return;
      ...
      } catch(error) {if(generation===feedbackGeneration)ctx.ui.setStatus('common-memory','Memory · 不可用 · /memory');}
    ...
    pi.on("session_start", (event,ctx)=>{
      ...
      safe(r=>r.start(ctx.sessionManager.getSessionId(),branchUsers(ctx.sessionManager.getBranch()),ctx.cwd));
    ...
    pi.on("input", (event,ctx)=>{
      safe(r=>{if(!ctx.hasPendingMessages())void r.cancelInputs(ctx.sessionManager.getSessionId(),ctx.cwd);return r.input({sessionId:ctx.sessionManager.getSessionId(),cwd:ctx.cwd,text:event.text,source:event.source,parentEntryId:ctx.sessionManager.getLeafId(),hasUnsupportedContent:(event.images?.length??0)>0,...(event.streamingBehavior?{streamingBehavior:event.streamingBehavior}:{})});});
    ...
    pi.on("message_end", (event,ctx)=>{
      if (event.message.role !== "user") {
        ...
        setImmediate(()=>{if(runtime&&ctx.sessionManager.getSessionId()===sessionId)safe(async r=>{await r.bind(sessionId,branchUsers(ctx.sessionManager.getBranch()),ctx.cwd);await r.context(sessionId,branchContext(ctx.sessionManager.getBranch()),ctx.cwd);});});
        return;
      }
      const text = messageText(event.message.content); if (!text) return;
      const unsupported=hasNonTextContent(event.message.content);
      safe(r=>r.delivered(ctx.sessionManager.getSessionId(),text,event.message.timestamp,unsupported,ctx.cwd));
    ...
    pi.on("agent_settled", (_event,ctx)=>safe(async r=>{const id=ctx.sessionManager.getSessionId();await r.bind(id,branchUsers(ctx.sessionManager.getBranch()),ctx.cwd);await r.context(id,branchContext(ctx.sessionManager.getBranch()),ctx.cwd);await r.settled(id,branchUsers(ctx.sessionManager.getBranch()),branchInterrupted(ctx.sessionManager.getBranch())?'interrupted':'settled',ctx.cwd);}));
    pi.on("session_before_switch", (_event,ctx)=>{bind(ctx);safe(r=>r.cancelInputs(ctx.sessionManager.getSessionId(),ctx.cwd));});
    pi.on("session_before_compact", (_event,ctx)=>{bind(ctx);});
    pi.on("session_before_tree", (_event,ctx)=>{bind(ctx);safe(r=>r.cancelInputs(ctx.sessionManager.getSessionId(),ctx.cwd));});
    pi.on("session_shutdown", async (event,ctx)=>{ unregisterInstance();feedbackGeneration++;if(feedbackTimer)clearInterval(feedbackTimer);feedbackTimer=undefined;uiContext=undefined;if(ctx.hasUI)ctx.ui.setStatus('common-memory',undefined);if(event.reason==='quit'){try{snapshots.delete(snapshotKey(ctx));}catch{/* unconfigured */}}if(runtime){try{const id=ctx.sessionManager.getSessionId();await runtime.bind(id,branchUsers(ctx.sessionManager.getBranch()),ctx.cwd);await runtime.context(id,branchContext(ctx.sessionManager.getBranch()),ctx.cwd);await runtime.cancelInputs(id,ctx.cwd);if(event.reason==='quit')await runtime.end(id,ctx.cwd);}finally{await runtime.shutdown();runtime=undefined;config=undefined;}} });
    ...
    pi.registerCommand("memory-flush",{description:"Queue Common Memory maintenance",handler:async (_args,ctx)=>{if(runtime)await runtime.bind(ctx.sessionManager.getSessionId(),branchUsers(ctx.sessionManager.getBranch()),ctx.cwd);await service.flush(host(ctx));await updateFeedback();}});
    pi.registerCommand('memory',{description:'Open Common Memory: browse, adjust, import and processing status',handler:async(_args,ctx)=>{
      ...
      await openMemoryPanel(ctx,service,()=>refresh(ctx),async()=>{if(runtime)await runtime.bind(ctx.sessionManager.getSessionId(),branchUsers(ctx.sessionManager.getBranch()),ctx.cwd);await service.flush(host(ctx));},()=>{if(generation!==feedbackGeneration)throw new Error('CANCELLED');});await updateFeedback();
    }});
    pi.registerTool({name:'memory_status',label:'Memory status',description:'Discover current authorized scopes and capabilities, inspect body-free queue state, or check an original importId/requestId in this Pi session. Automatic capture already submits user turns: do not duplicate them. Processed is not proof of retention. Follow next; prefer /memory for interactive recovery, not repeated polling.',promptSnippet:'Check memory permissions, processing and recovery.',parameters:Type.Object({importId:Type.Optional(Type.String({pattern:'^[A-Za-z0-9_-]{1,128}$'})),requestId:Type.Optional(Type.String({pattern:'^[A-Za-z0-9_-]{1,128}$'}))}),execute:async(_id,input,_signal,_update,ctx)=>{
      try {const value=await service.status(host(ctx),input);return {content:[{type:'text',text:JSON.stringify(value)}],details:value};}catch(error){throw nativeFailure(error);}
    }});
    pi.registerTool({name:'memory_init',label:'Import memory material',description:'Only when the user explicitly requests importing existing material. Preserve sources, dates, conditions, uncertainty and gaps; do not preselect long-term value or submit this session\'s execution status. Material stays agent-reported, never authenticated user speech. Requires configured agent_observation authorization AND native user confirmation of this payload. Keep importId and payload unchanged on retry. Accepted means queued, not remembered. Never duplicate automatically captured user turns.',parameters:Type.Object({importId:Type.String({pattern:'^[A-Za-z0-9_-]{1,128}$'}),contextId:Type.String({maxLength:160}),sourceLabel:Type.String({pattern:IMPORT_LABEL_PATTERN.source}),basis:StringEnum([...IMPORT_BASES]),understanding:Type.String({minLength:1}),gaps:Type.Optional(Type.String())}),execute:async(_id,input,signal,_update,ctx)=>{
      ...
      try {
        ...
        const result=await service.import(host(ctx),payload,signal);await updateFeedback();return {content:[{type:'text',text:JSON.stringify(result)}],details:result};

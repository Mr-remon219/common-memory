...
export function renderCodexConfig(env: NodeJS.ProcessEnv = process.env): string {
  if (process.platform === 'win32') throw new Error('codex-config requires Codex and Common Memory in the same POSIX environment (including WSL)');
  ...
  return [
    ...
    '[features]', 'hooks = true', '',
    ...['UserPromptSubmit', 'SessionStart'].flatMap(event => [
      `[[hooks.${event}]]`,
      ...(event === 'SessionStart' ? ['matcher = "^compact$"'] : []),
      `[[hooks.${event}.hooks]]`, 'type = "command"',
      `command = ${JSON.stringify(command)}`,
      'async = false', 'timeout = 5', 'additionalContextLimit = 0', '',
    ]),
  ].join('\n');
}

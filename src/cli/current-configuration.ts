...
export function currentConfiguration(config: CommonMemoryConfig): string {
  ...
  return terminalText([
    ...
    `Complete source: ${inputLimits(current.disclosure).maxSourceBytes ?? 'Unlimited'} bytes; Max Input also caps total serialized provider payload (system/tools/context included).`,
    ...inputLimits(current.disclosure).deprecatedLimits,
    `Max Output: ${current.remote.maxOutputTokens == null ? 'Unlimited' : `${current.remote.maxOutputTokens} tokens (Common Memory cap)`}`,
    `Agent turns: ${current.remote.maxAgentTurns ?? 64}; whole-task deadline: none; automatic retries: at most ${Math.min(5,current.scheduler.maxAttempts)}`,
    'Network progress: headers 30 seconds; stream idle 120 seconds (reset on progress).',
    'Unlimited means no additional Common Memory cap; provider limits still apply.',

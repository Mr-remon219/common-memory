...
export function renderMcpConfig(config: CommonMemoryConfig, options: McpConfigOptions, env: NodeJS.ProcessEnv = process.env): string {
  ...
  const lines = [
    ...
    ``,
    `# Desktop session with local Codex-host MCP access (including the observed Work-local session): Init only.`,
    `# The host approval setting below covers non-read-only tools; approval does not verify imported claims.`,
    `# Record materials actually visible to the agent; a product/mode name does not establish memory source or coverage.`,
    `# This connection does not export all ChatGPT memory. Review the destination with common-memory show.`,

...
v0.3 使用所有者确认的 MIT 许可证。npm 版本是 **0.3.7**，GitHub tag 是 **v0.3.7**，
...
npm run test:wsl -- --registry-version 0.3.7
```
...
```sh
npm view common-memory-core@0.3.7 version dist.integrity
npm install -g common-memory-core@0.3.7
...
npm 发布成功并完成本地 registry 检查后，创建 GitHub Release `v0.3.7`。它会触发

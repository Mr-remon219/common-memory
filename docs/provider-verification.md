...
API Key 只从输入配置同目录、经 TUI 配置的私有 `.env` 读取，忽略进程环境中的同名 Key。脚本仅将选中的 Key 和所需的私有网络设置写入临时 home 的 0600 `.env`，不复制其他条目；重启只读验收前删除临时凭据，失败收尾也清理。原 `.env` 不修改。自定义外部网络引用仍按网络层规则处理。配置与报告只包含条目名，不包含 Key、代理凭据或原始 Provider 错误正文。
...
1. 依据官方文档与账户权限，确定**端点 + 精确模型 + 请求模式**。在隔离的 `COMMON_MEMORY_HOME` 中用 TUI 配置模型、Key 与网络；Key 只写入该目录的私有 `.env`，不写入 JSON 配置。
2. 使用该目录的 `config.json` 执行 smoke；不注入 API Key 环境变量，无需再写或复制一份 smoke 脚本。

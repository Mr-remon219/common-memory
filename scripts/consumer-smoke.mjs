...
const wsl = args[0] === '--wsl';
if (wsl) { args.shift(); (await import('../tests/wsl/installed.mjs')).requireWsl(); }
if (args.length && !(args.length === 2 && args[0] === '--registry-version' && /^\d+\.\d+\.\d+$/.test(args[1]))) {
  throw new Error('Usage: consumer-smoke.mjs [--wsl] [--registry-version <exact-version>]');
}
...
try {
  ...
  if (wsl) {
    ...
    assert.equal(manifest.version, registryVersion ?? JSON.parse(readFileSync(join(root, 'package.json'), 'utf8')).version);
    assert.equal(run([join(pkg, 'dist/cli/main.js'), '--version']).trim(), manifest.version);
    await (await import('../tests/wsl/installed.mjs')).verifyWsl({ pkg, temp, root });
  } else {
